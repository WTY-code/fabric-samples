...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    # # > organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/full-ca-chain.crt

    # for org in org1 org2 org3; do
    #   for peer in peer0 peer1 peer2; do
    #     domain="$peer.$org.example.com"
    
    #     src1="organizations/peerOrganizations/$org.example.com/peers/$domain/tls/server.crt"
    #     src2="organizations/peerOrganizations/$org.example.com/tlsca/tlsca.$org.example.com-cert.pem"    
    #     dest="organizations/peerOrganizations/$org.example.com/peers/$domain/full-ca-chain.crt"
    ...
    #       echo "Generated: $dest"
    #     else
    #       echo "ERROR: Failed to generate $dest" >&2
    #     fi
    #   done
    # done
...
function deployCC() {
  scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $CC_INSTALL_ALL_PEERS
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # Bring down the network, deleting the volumes
    # Remove all peer volumes for all organizations
...
# Chaincode installation flag for all peers
CC_INSTALL_ALL_PEERS="false"

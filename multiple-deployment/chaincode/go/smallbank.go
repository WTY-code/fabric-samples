...
func (t *SmallbankChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "query_all_accounts":
		return t.QueryAllAccounts(stub, args)
	default:
...
func (t *SmallbankChaincode) QueryAllAccounts(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for iterator.HasNext() {
		...
		var account Account
		err = json.Unmarshal(responseRange.Value, &account)
		if err != nil {
			return systemerror(err.Error())
		}
	...
	return shim.Success(accountsBytes)
}
